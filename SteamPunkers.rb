...
class SteamPunkers < DiceBot
  setPrefixes([
     '\d+SP.*',
	 'PAT', 'PST', 'PCT',
	 'NIT', 'NAT', 'NFT', 'NGT', 'NST', 'NJT',
	 'PTT', 'PGT', 'PNT', 'PDT', 'PET', 'PCT',
	 'BGT', 'BAT', 'BET', 'BCT', 'BST', 'BWT',
	 'FST', 'FPT', 'RST', 'RPT', 'EST', 'EPT',
	 'ODT', 'OST', 'OIT', 'OFT'
  ])
...
・判定コマンド　(nSP)
　n個のD6を振って5以上が出るたびに成功数+1。
  6が出たらクリティカル。成功数に6の個数を追加。
　出目が全部1ならファンブル。
・各種表 
...
INFO_MESSAGE_TEXT
  ...
  def checkRoll(string)
    ...
    unless( operator.nil? )
      modify = operator.to_i
    end

    result = ""

    dice_str, total_suc, isFumble = checkRollLoop(roll_times, target, critical, fumble)
    ...
    result += "【#{string}】 ＞ #{dice_str} ＞ 成功数#{total_suc}"
  ...
  def checkRollLoop(roll_times, target, critical, fumble)
    dice_str = ''
    isFumble = false
    total_suc = 0
	fumble_count = 0
    ...
    roll_times.times do |i|
      ...
      dice_suc = 0
      dice_suc = 1 if(target <= (d1))
      dice_suc += 1 if(critical <= d1)
	...
	if(roll_times == fumble_count) # ファンブルの確認
      isFumble = true
	end

    return dice_str, total_suc, isFumble
  end
  ...
  def rollTableCommand(command)
    ...
    when "PAT"
	  name = "年齢表"
	  table = getProfileAgeTable()
	when "PST" 
	  name = "性別表"
	  table = getProfileSexTable()
	when "PCT"
	  name = "国籍表"
	  table = getProfileCountryTable()
	when "NIT"
	  name = "名前表・イギリス(男性名/女性名/名字)"
	  table = getNameEnglandTable()
	when "NAT"
	  name = "名前表・アメリカ(男性名/女性名/名字)"
	  table = getNameAmericaTable()
	when "NFT"
	  name = "名前表・フランス(男性名/女性名/名字)"
	  table = getNameFranceTable()
	when "NGT"
	  name = "名前表・ドイツ(男性名/女性名/名字)"
	  table = getNameGermanyTable()
	when "NST"
	  name = "名前表・ソビエト(男性名/女性名/名字)"
	  table = getNameSovietTable()
	when "NJT"
	  name = "名前表・日本(男性名/女性名/名字)"
	  table = getNameJapanTable()
	when "PTT"
	  name = "過去表『トゥルース』"
	  table = getPastTruthTable()
	when "PGT"
	  name = "過去表『ガーディアン』"
	  table = getPastGuardianTable()
	when "PNT"
	  name = "過去表『ノーブル』"
	  table = getPastNobleTable()
	when "PDT"
	  name = "過去表『デヴォート』"
	  table = getPastDevoteTable()
	when "PET"
	  name = "過去表『エセティック』"
	  table = getPastEstheticsTable()
	when "PCT"
	  name = "過去表『チャレンジ』"
	  table = getPastChallengeTable()
	when "BGT"
	  name = "経緯表『ギガントアームズ』"
	  table = getBackgroundGATable()
	when "BAT"
	  name = "経緯表『アーマードリム』"
	  table = getBackgroundARTable()
	when "BET"
	  name = "経緯表『エッジモービル』"
	  table = getBackgroundEMTable()
	when "BCT"
	  name = "経緯表『クロノウェポン』"
	  table = getBackgroundCWTable()
	when "BST"
	  name = "経緯表『スパイテック』"
	  table = getBackgroundSTTable()
	when "BWT"
	  name = "経緯表『スチームウェア』"
	  table = getBackgroundSWTable()
	when "FST"
	  name = "特徴表S"
	  table = getFeatureSteamTable()
	when "FPT"
	  name = "特徴表P"
	  table = getFeaturePunkersTable()
	when "RST"
	  name = "関係性表S"
	  table = getRelationshipSteamTable()
	when "RPT"
	  name = "関係性表P"
	  table = getRelationshipPunkersTable()
	when "EST"
	  name = "感情表S"
	  table = getEmotionSteamTable()
	when "EPT"
	  name = "感情表P"
	  table = getEmotionPunkersTable()
	when "ODT"
	  name = "災厄表"
	  table = getOtherDisasterTable()
	when "OST"
	  name = "場面表"
	  table = getOtherSceneTable()
	when "OIT"
	  name = "交流表"
	  table = getOtherInteractTable()
	when "OFT"
	  name = "激憤表"
	  table = getOtherFuryTable()
    end

	result = createTableSentence(name, table)

    return result.join("\n")
  end

  def getCursorOnTable()
    d1, = roll(1, 6)
	d2, = roll(1, 6)
	num = 1
  ...
  def createTableSentence(name, table)
    result = []
	d1, d2, num = getCursorOnTable()
	main = table[num-1]
	result.push("#{name}:[#{d1},#{d2}]:#{main}")
    return result
  end

  def getProfileAgeTable()
    table = [
  ...
  def getRelationshipPunkersTable()
    table = [
